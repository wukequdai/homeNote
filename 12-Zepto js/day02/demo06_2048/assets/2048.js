...
function setWidthHeight(){
  //得到实际的宽度和高度
  //cell_width cell_height 每一个小方块的宽、高
  //base_marginLeft base_marginTop 相邻的方块之间的左右边距、上下之间的距离

  //width = 4*cell_width+5*(cell_width/4)
  //height = 4*cell_height+5*(cell_height/4)

  var width = window.innerWidth;
  var height = window.innerHeight-$('#score').height();

   cell_width = width*(4/21);
   cell_height = height*(4/21)
   base_marginLeft = cell_width/4;
   base_marginTop = cell_height/4;
  ...
  for(var r=0;r<4;r++)
  {
    for(var c=0;c<4;c++)
    {
      console.log($('[id $= "'+r+c+ '"]'));
      $('[id $= "'+r+c+ '"]').css({
        left:base_marginLeft*(c+1)+cell_width*c,
        top:base_marginTop*(r+1)+cell_height*r
      })
    }
  }
...
*/
  ...
  start: function () {//开始游戏方法
    ...
    grid.on('swipeUp', function () {
      this.moveUp();
    }.bind(this));
    grid.on('swipeDown', function () {
      this.moveDown();
    }.bind(this));
    ...
    for (var r = 0; r < this.RN; r++) {
      for (var c = 0; c < this.CN; c++) {
        ...
        if (this.data[r][c] == 0) {
          ...
          //如果有内容了，上一步设置完className和要显示的内容之后
          //重新设置top和left的值
          $('[id $= "'+r+c+ '"]').css({
            left:base_marginLeft*(c+1)+cell_width*c,
            top:base_marginTop*(r+1)+cell_height*r
